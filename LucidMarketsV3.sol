...
contract LucidMarketsV4 is Ownable {
    ...
    // 📦 CONFIG (Updated for V4)
    struct MarketConfig {
        string question;
        string metadataURI;
        uint256 endTime;        // 🛑 Betting Closes Here (e.g., 4:45 PM)
        uint256 resolutionTime; // 🔮 Oracle Checks Price Here (e.g., 5:00 PM)
        int256 targetPrice;
        address oracleFeed;
    ...
    function createMarket(MarketConfig calldata params) external onlyOwner {
        ...
        // 🛡️ Safety: You can't resolve BEFORE betting ends
    ...
    function placeBet(uint256 _id, bool _isYes, uint256 _amt, address _ref) external payable nonReentrant {
        ...
        // 🛑 Betting Deadline Check: Uses 'endTime' (4:45 PM)
    ...
    function resolveMarket(uint256 _id, uint80 _roundId) external onlyOwner {
        ...
        // 🔮 Oracle Check: Uses 'resolutionTime' (5:00 PM)
    ...
    function claimWinnings(uint256 _id) external nonReentrant {
        MarketStatus storage s = marketStatuses[_id];
        BetInfo storage bet = userBets[_id][msg.sender];
        require(s.resolved && !bet.claimed && !s.cancelled, "Invalid claim");
    ...
    // Sweep: Uses 'resolutionTime' + 365 days
    ...
    // Emergency Refund: Uses 'resolutionTime' + 3 days
